...
The most important factor for determining the learning rate, as it scales the step length of the gradient descent procedure. To still ensure sufficient learning, the number of iterations is usually increased to account for a reduced learning rate. Another feature of XGBoost is the so called early stopping, this method help to determine the optimal number of iterations to build a model that is not overfitted or exhibits a great amount of bias. Early stopping occurs when the models performace on teh validation set plat<mark style="background: #FFB8EBA6;">eus</mark>
